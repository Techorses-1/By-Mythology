...
// 📦 CREATE ORDER (from checkout) - FIXED VERSION WITH EMAIL
router.post('/create', auth, async (req, res) => {
    try {
        ...
        try {
            ...
            const userEmailHtml = getOrderConfirmationEmail(order, userEmailAddress, userName);
            await sendEmail({
                to: userEmailAddress,
            ...
            const adminEmailHtml = getAdminNewOrderEmail(order, userEmailAddress, userName);
            await sendEmail({
...
// ✏️ UPDATE ORDER STATUS (admin) - WITH EMAIL
router.put('/:orderId/status', auth, async (req, res) => {
    try {
        ...
        if (status === 'cancelled') {
            ...
            order.timeline.processingAt = new Date();
        ...
        try {
            ...
            const userEmailHtml = getOrderStatusUpdateEmail(order, userEmailAddress, userName, oldStatus, status);
            await sendEmail({
                to: userEmailAddress,
            ...
            console.log(`✅ Status update email sent to ${userEmailAddress}`);
        } catch (emailError) {
...
// ❌ CANCEL ORDER (user request) - WITH EMAIL
router.put('/:orderId/cancel', auth, async (req, res) => {
    try {
        ...
        try {
            ...
            const userEmailHtml = getOrderCancelledUserEmail(order, userEmailAddress, userName);
            await sendEmail({
                to: userEmailAddress,
            ...
            const adminEmailHtml = getAdminOrderCancelledEmail(order, userEmailAddress, userName);
            await sendEmail({
                to: process.env.EMAIL_USER,
